...
然后在服务器上clone一下你新建的分支，通过`-b`选择分支，例如
...
之后按照上面的操作，在这个终端里安装一下nodejs和导入一下GitHub的信息，只不过不用再生成SSH密钥了（`ssh-keygen`和`cat`这两个命令不用了，也不用导入密钥了），但是git config命令要改成这样
...
之后要在终端里npm install，由于我是国内服务器，就用cnpm install了（要使用cnpm的话，要执行一下`npm install -g cnpm`）
